...
class TestP8FileTransformation(BaseTest):
    ...
    def testGameDescriptionCommentsCanBeAdded(self):
        ...
        contents = contents.replace("--{gamename}\n--{authorinfo}\n", "")
        copyLoc: Path = self.getTempFilePath(
            TempFileEnum.GAME_CART_TEST_FILE_TRANSFORMED_COPY_LOCATION
        )
        copyLoc.write_text(data=contents)
